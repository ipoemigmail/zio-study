package ipoemi.zio.study._07_testing_effect

import ipoemi.zio.study._07_testing_effect.db.Database
import zio._
    ...
    extends DefaultRunnableSpec(
      suite("Testing Effect")(
        ...
        testM("Environment Effect") {
          ...
          val testDb = for {
            _ <- TestDatabase.database.setTestData(Map(userId -> UserProfile(userId, "user0")))
          } yield TestDatabase

          assertM(lookedupProfile.provideM(testDb), equalTo(UserProfile(userId, "user0")))
        }
      )
    )
